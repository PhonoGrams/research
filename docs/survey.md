...
5. **Phonetics are a separate layer.** Soundex / Double Metaphone / ALINE / Arabic phonetization do a different job (sound-alike, cross-script). Watchman already has Soundex boost and Arabic phonetic mapping. Putting a second, worse phonetic rewriter inside Soft-Bisim (the old `language.go`) was a mistake.

## Paper-by-paper, short

**Kondrak 2005 (★ added).** The foundation. N-DIST / N-SIM, affixing, positional n-gram cost, proof that Levenshtein and LCS are n=1. Implement the n=2 cases as `KondrakDistance` / `Bisim` and treat Soft-* as a cost-scale swap.

**Kondrak & Dorr 2004 (★ added).** BI-SIM on confusable drug names; recall-at-rank evaluation that Millán-Hernández copies. Combined measure (Prefix + NED + Bisim + Aline) beats any single measure. That is the strongest "how to integrate" result in the whole pile: **blend**, do not replace.
...
**Hadwan 2021 (Soft-Bidist).** Softens BI-DIST. Table 4 spelling pairs are a useful unit test and a poor Watchman test. Peer review (Brew, PeerJ) already flagged an unclear recurrence and informal weight search. We follow the written recurrence plus the authors' C# case order, not the sticky-`cost2` bug in that C#.
...
**Kasyap et al. 2024 (★ added).** Privacy-preserving fuzzy name matching for financial intelligence (Turing / Warwick / HSBC). Character n-grams → MinHash → FHE. Confirms n-grams are what banks actually ship when they cannot send raw names. Not an accuracy paper; a systems paper. No change to the scorer, but it is why a clean `Similarity(a, b) float64` API matters (it is the inner kernel of a PSI/FHE scheme too).

**OpenSanctions Pairs 2026 (★ added).** 755,540 expert-labeled entity pairs, sanctions/PEP/OSINT, n-gram blocking in `nomenklatura`. Two Watchman takeaways: (1) name-only matching fails on common names that share country and program (their 0.98 false positive); Watchman's identifier and country features already know this. (2) This is the eval set to use before calling Soft-Bisim "better than Jaro-Winkler" on sanctions.
...
## What we discarded from the old Go trees

- LCS-as-wt8/wt9 in `soft_bigram` (not in the paper).
- Full-matrix `[][]string` bigrams and `math.Max` on ints.
- `soft-bisim` `language.go` with two `main` functions, duplicate `DetectLanguage`, and a `golang.org/x/text` language matcher pointed at the name string (that matcher expects BCP-47 tags, not "Schwarz").
- A "genetic algorithm" that was random mutation of weights with a fitness function of "average similarity on positive pairs" (no negatives → weights collapse toward always-similar).
- Phonetic rewrites (`ph→f`, vowel stripping) baked into the distance. They destroy OFAC spelling that Watchman is supposed to keep.
...
Implement Kondrak n=2 correctly, then the two soft scales, with a `Similarity` in `[0, 1]`, identity = 1, zero allocs on short names, and no phonetic cleverness. Fit weights later, on OpenSanctions Pairs, inside Watchman.
